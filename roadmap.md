...
| Version | Date     | Change |
| ------- | -------- | ------ |
| 0.9     | Jul 2026 | **Task 6 (Monthly overview) shipped — closes v0.1 Foundations.** `/ledger` is now the dashboard: income/expense/savings/budget-balance stat cards, expenses-by-group breakdown, and a budget-vs-actual list, all for a `?period=YYYY-MM` calendar month (defaults to the current month; `←`/`→` links shift it). Budget-vs-actual only covers subcategories — a top-level category used without any subcategory of its own can never accumulate actual spend (expenses always log against a subcategory, Task 5), so including it here would only ever show a misleading zero. "Budget balance" (a stat SPEC.md lists but never formally defines) is interpreted as total applicable budget minus total actual across every subcategory — distinct from "savings" (income minus expenses) — documented inline in `overviewActions.ts` since the SPEC text alone doesn't disambiguate it. Periods are plain calendar months; `month_start_day` isn't applied, consistent with how Tasks 3/4 already wrote their own 'YYYY-MM' periods. Savings doesn't yet net out jar contributions (Task 7-9 — jars don't exist yet). |
| 0.8     | Jul 2026 | **Task 5 (Expense logging) shipped.** `/ledger/expenses` — logs a daily expense against a subcategory (date, amount, optional note), edit, and soft-delete (`deleted_at`, LDG-08's "preferred for auditability" — rows stay for future history/export, just excluded from live totals via `isNull(deletedAt)`). The category picker only ever offers subcategories, never a bare top-level category, matching `ledger_transactions.category_id`'s own schema comment; server actions re-validate `parentId` isn't null even though the picker already filters it, since a tampered `categoryId` could otherwise slip a top-level id through. Every expense is written with `fx_mode: 'base'`, no rate, and `base_amount = amount` — same pre-FX placeholder pattern as Task 4's income entries, real conversion arrives with Task 10. Added `todayLocalDateOnly` to `app/_lib/formUtils.ts` (shared, not duplicated per-dialog) for the date field's default. |
| 0.7     | Jul 2026 | **Task 4 (Income sources & entries) shipped.** `/ledger/income` — create/edit income sources (name, kind `fixed`/`dynamic`, expected amount, currency, cadence `monthly`/`yearly`/`adhoc`), archive/unarchive; `/ledger/income/[sourceId]` records actual income entries per month (add/delete, optional note). Every entry is written with `fx_rate: '1'` and `base_amount = amount` regardless of the source's currency — live FX conversion doesn't exist until Task 10, and the schema was deliberately scaffolded (Task 0) so this doesn't need a migration later, only a real rate lookup swapped in. Renamed the shared dialog-form stylesheet `CategoryDialogs.module.css` → `DialogForm.module.css` (now used by category, budget, and income dialogs — the old name stopped describing its scope). |
| 0.6     | Jul 2026 | **Task 3 (Budgets) shipped.** `/ledger/budgets` — one inline form per category/subcategory sets its default (period = null) monthly budget in the user's base currency; `/ledger/budgets/[categoryId]` manages per-month overrides (add/delete), each keyed on `(category_id, period)` with a select-then-insert-or-update upsert (no unique index backs it — `ledger_budgets` never declared one, matching the schema as shipped in Task 0). Added `app/_lib/money.ts` (`majorToMinor`/`minorToMajor`, 2-decimal assumption) mirroring `sovereign-tally`'s `dollarsToCents`/`centsToDollars` — same precedent, currency-neutral naming since Ledger isn't dollar-only. |
| 0.5     | Jul 2026 | **Task 2 (Category tree) shipped.** `/ledger/categories` — create top-level categories (name + group), add subcategories under them (name only; a subcategory always inherits its parent's `group` at creation, denormalized onto its own row so rollup queries never need a parent join), rename either level, archive/unarchive either level independently (no cascade — a deliberate scope decision, not a gap). `app/_lib/db.ts` now holds the shared `getContext`/`Db`/`ActionResult` used across actions files, split out of `actions.ts` since a `'use server'` file may only export async functions and this task added a second actions file (`categoryActions.ts`). |
| 0.4     | Jul 2026 | **Task 1 (Settings) shipped.** `/ledger/settings` — base currency, display currency (nullable, defaults to base), month-start day (1–28), backed by `ledger_settings` upsert via `onConflictDoUpdate` on the `(tenant_id, user_id)` composite PK. Follows the healthlog/docs `ActionResult` + `useActionState` convention (not wallet's manual-`useTransition` variant). Added `app/error.tsx` (every plugin ships one, per the design skill's pre-merge checklist) and a `/ledger` index with a Settings entry card, since the placeholder `EmptyState` had nowhere to link to. Currency list is plugin-local (`app/_lib/currencies.ts`, 16 ISO 4217 codes) — no shared currency constant exists anywhere in the platform. **Also fixes a Task 0 scaffold defect**: `db/schema.ts` sat at the plugin root, outside `app/` — the runtime only composes a plugin's `app/` tree into its route table (`scripts/generate-registry.ts`), so any server action importing table objects from `../../db/schema` failed to resolve at build time the moment real DB access landed. Query schema moved to `app/_db/schema.ts` (matching `sovereign-healthlog`'s precedent); root `db/schema.ts` is now a one-line `export * from '../app/_db/schema'` shim for drizzle-kit/tooling. `db/schema.postgres.ts` (migration-generation only, never imported by app code) is unaffected. |
