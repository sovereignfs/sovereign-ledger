 ...
 *
 * This is the runtime query schema application code imports. It lives under
 * `app/` — not the plugin-root `db/` — because the Sovereign runtime only
 * composes a plugin's `app/` tree into the Next.js route table (see
 * `scripts/generate-registry.ts`'s `composePlugins`); anything outside `app/`
 * is invisible at request time. `db/schema.ts` re-exports this file so
 * drizzle-kit and any repo-root tooling can still find it. Same pattern as
 * `sovereign-healthlog`'s `app/_db/schema.ts`.
 *
